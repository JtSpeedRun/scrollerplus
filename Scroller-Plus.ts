...
namespace scrollerPlus {
    ...
    class LayerState {
        ...
        getLayer(layer: number) {
            layer = Math.constrain(layer | 0, 0, 10);
            while (this.layers.length < layer + 1) this.layers.push(new ScrollerState());
            return this.layers[layer];
        }

        update() {
            for (const layer4 of this.layers) {
                layer4.update(this.lastCameraX, this.lastCameraY)
            }
    ...
    class ScrollerState {
        ...
        draw(target: Image) {
            const bg2 = this.image || scene.backgroundImage();

            if (this.xOffset) {
                if (this.yOffset) {
                    target.drawTransparentImage(bg2, (this.xOffset | 0) - bg2.width, (this.yOffset | 0) - bg2.height);
                    target.drawTransparentImage(bg2, (this.xOffset | 0) - bg2.width, (this.yOffset | 0));
                    target.drawTransparentImage(bg2, (this.xOffset | 0), (this.yOffset | 0) - bg2.height);
                    target.drawTransparentImage(bg2, (this.xOffset | 0), (this.yOffset | 0));
                }
                else {
                    target.drawTransparentImage(bg2, (this.xOffset | 0) - bg2.width, 0);
                    target.drawTransparentImage(bg2, (this.xOffset | 0), 0);
                }
            }
            else if (this.yOffset) {
                target.drawTransparentImage(bg2, 0, (this.yOffset | 0) - bg2.height);
                target.drawTransparentImage(bg2, 0, (this.yOffset | 0));
            }
            else {
                target.drawTransparentImage(bg2, 0, 0)
            }
        }
    }
    ...
    export function scrollBackgroundWithSpeed(vx: number, vy: number, layer = 0) {
        const currentState2 = state().getLayer(layer);
        currentState2.currentXSpeed = vx;
        currentState2.currentYSpeed = vy;
        currentState2.cameraScrolling = false;
    }
    ...
    export function setCameraScrollingMultipliers(xMultiplier = 1, yMultiplier = 1, layer = 0) {
        const currentState3 = state().getLayer(layer);
        currentState3.cameraScrolling = true;
        currentState3.cameraXMultiplier = xMultiplier;
        currentState3.cameraYMultiplier = yMultiplier;
    }
    ...
    export function setBackgroundScrollOffset(x: number, y: number, layer = 0) {
        const currentState4 = state().getLayer(layer);
        currentState4.xOffset = x;
        currentState4.yOffset = y;
    }
